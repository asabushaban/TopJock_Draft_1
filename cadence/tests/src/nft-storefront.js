...
import { deployTopJockNFT, setupTopJockNFTOnAccount } from "./top-jock-nft";

/*
 * Deploys TopJockNFT and NFTStorefront contracts to TopJockAdmin.
...
export const deployNFTStorefront = async () => {
	...
	await deployTopJockNFT();

	return deployContractByName({ to: TopJockAdmin, name: "NFTStorefront" });
};
...
export const setupStorefrontOnAccount = async (account) => {
	// Account shall be able to store TopJock Items
	await setupTopJockNFTOnAccount(account);
