...
const HeaderContainer = ({children}) => {
  return (
    <div className="bg-black text-white text-md font-bold text-center py-3 px-2">
      {children}
    </div>
  )
}
...
export default function HeaderMessage() {
  ...
  const checkIsServiceAccountInitialized = () => {
    isAccountInitializedTx(publicConfig.flowAddress).then(data => {
      setIsServiceAccountInitialized(data.TopJockNFT && data.TopJockNFTMarket)
    })
  }
      ...
      <HeaderContainer>
        {isServiceAccountLoggedIn ? (
          <>
            ...
            the Service Account to mint TopJockNFT.
          </>
        ) : (
          <>
            {currentUser ? (
  ...
  return (
    <HeaderContainer>
      <span className="mr-3 text-sm">💻</span>TopJockNFT is a demo application
