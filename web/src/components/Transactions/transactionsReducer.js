...
import {
  EVENT_ITEM_MINTED,
  EVENT_TOP_JOCK_DEPOSIT,
  getTopJockEventByType,
...
const newUpdateTransactionPayload = tx => {
  ...
  try {
    const depositEvent = getTopJockEventByType(
      tx.data.events,
      EVENT_TOP_JOCK_DEPOSIT
    )
    if (!!depositEvent) {
  ...
  try {
    const mintEvent = getTopJockEventByType(tx.data.events, EVENT_ITEM_MINTED)
  } catch (e) {
    console.error(e)
  }
